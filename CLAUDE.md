...
- **La onda es el único ornamento, y es una pausa, no un borde**: parte el hueco entre secciones y hace de `hr` en la prosa. Vive en `mask` para que el color siga siendo un token. Lo que es chrome -barra, pie, hairlines entre filas- sigue recto, y los enlaces también; una onda más -bajo un título, en el wordmark, bajo un enlace- la convierte en plantilla.
