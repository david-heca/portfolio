...
- **Nada se sale de la columna** -salvo el aviso de idioma, que es una notificación y no parte de la página-. Sin armazón, sin líneas a sangre, sin marcas de registro, sin reveal al hacer scroll: cada cosa que había que sostener con un pseudoelemento era decoración. Las esquinas van a escuadra (`--radius` y `--radius-sm` en 0); se salvan las viñetas.
- **La onda es el único ornamento, y es una pausa, no un borde**: parte el hueco entre secciones, hace de `hr` en la prosa y, a media escala, subraya `.link`. Vive en `mask` para que el color siga siendo un token. Lo que es chrome -barra, pie, hairlines entre filas- sigue recto; una onda más -bajo un título, en el wordmark- la convierte en plantilla.
...
- **Las listas son la unidad, y una fila es un título y una línea.** `.rows`/`.row` es una fila de raíl más cuerpo con hairline entre filas; `.thumb` es la única caja para imagen y `--thumb` su único tamaño. Delante del título va una fecha en mono o una miniatura, y debajo una sola línea: sin fichas de specs, sin stack, sin estado, sin logros plegados. Lo que un elemento necesite contar de más va a una nota. Un patrón que aparece en tres sitios ya es una primitiva, y vive en `global.css`.
...
- **Los iconos son lo que no es texto.** Phosphor monocromático heredando `currentColor`, y solo donde señalan algo: un canal, un dato, una flecha, un estado. **El glifo va en la tinta de su palabra**, sin regla de color propia; el control que es solo icono -tema, canales del pie, cerrar el aviso- va en ink-2 como una palabra de la barra. En ink-3 un icono se desvanece. **Nunca un glifo como icono** (`↗`, `→`): algunos navegadores los resuelven contra una fuente de emoji.
...
- **Un enlace sin subrayado no se ve.** Sin tono que lo separe del texto, el subrayado es lo único que lo señala. `.link` es el enlace de una línea que es una acción -«ver todas», volver- y lleva la onda; un dato que es enlace -el correo, `.link--plain`- y el enlace dentro de la prosa llevan raya recta, porque la onda señala movimiento y un párrafo con cinco ondas es adorno. Vale la inversión -fondo tinta, texto papel- para la acción primaria.
