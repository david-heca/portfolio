...
- **Las listas son la unidad.** `.rows`/`.row` es una fila de raíl más cuerpo con hairline entre filas; `.facts` es una fila de datos con glifo delante; `.thumb` es la única caja para imagen y `--thumb` su único tamaño. Un patrón que aparece en tres sitios ya es una primitiva, y vive en `global.css`.
- **Los iconos son lo que no es texto.** Phosphor monocromático heredando `currentColor`, y solo donde señalan algo: un canal, un dato, una flecha, un estado. **Nunca un glifo como icono** (`↗`, `→`): algunos navegadores los resuelven contra una fuente de emoji.
- **Estar en una ruta -`[aria-current="page"]`- se marca invirtiendo el enlace**, que es el mismo gesto que la acción primaria.
...
- **Un enlace sin subrayado no se ve.** Sin tono que lo separe del texto, el subrayado es lo único que lo señala: `.link`. Vale la inversión -fondo tinta, texto papel- para la acción primaria.
...
- **Ninguna sección lleva etiqueta en versalitas sobre el título.** Sobre un título es autoridad editorial prestada.
- **Un rol por tipografía.** Display para el wordmark; `--font-text` para leer y para la UI; `--font-mono` solo para etiquetas en mayúsculas con tracking, tags y cifras. Lo que se lee como frase va en la sans aunque contenga números.
